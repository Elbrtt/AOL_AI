...
class SmartWasteAnalyzer:
    ...
    def generate_waste_recommendations(self, row, predicted_spoilage_chance):
        ...
        # Rekomendasi berdasarkan temperature sensitivity
        if row.get('TemperatureSensitive', False):
            recommendations.append({
                'prioritas': 'SEDANG',
                'kategori': 'Temperature Control',
                'pesan': 'Produk temperature-sensitive memerlukan cold chain management',
                'aksi': 'Pastikan temperature dijaga, cek equipment pendingin'
            })
